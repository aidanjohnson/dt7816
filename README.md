...
8. To synchronise the board's clock to official US (NIST) time, after the Operation Instructions, follow the steps at this [wiki page](https://wiki.archlinux.org/index.php/Internet_sharing), substituting `enp4s0f1` for `net0` and `wlp3s0` for `internet0` in addition to substituting the arbitrary IP addresses of `192.168.123.0` with `10.0.0.0`, `192.168.123.100` with `10.0.0.1` and `192.168.123.201` with `10.0.0.2`. After this shares the internet connection of the host computer to the client board, set up the NTP server and client by running the command `ntpdate time.nist.gov` on the server host and `ntpdate 10.0.0.1` on the server client (consult [this page](http://www.ubuntugeek.com/network-time-protocol-ntp-server-and-clients-setup-in-ubuntu.html) for assistance).

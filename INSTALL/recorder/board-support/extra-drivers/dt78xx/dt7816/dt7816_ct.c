...
struct dt7816_ct
{
    ...
    ct_gate_t gate;
    uint8_t dout;  //divider output bit or one_shot output bit
};
...
//File operations supported by DT7816 counter timer subsystem
...
/*****************************************************************************
 * Relinquish control of the digital output used by counter timer
 * @param dev
 * @param dout
 */
static inline void _ct_out_free(struct device *dev, uint8_t dout)
{
    dt78xx_fpga_rmw(dev, 
                    (dout < 4) ? FPGA_REG_DOUT_GPIO1 : FPGA_REG_DOUT_GPIO2, 
                    MASK_DOUT_GPIO_MODE(dout, ~DOUT_GPIO_MODE_DOUT), 
                    MASK_DOUT_GPIO_MODE(dout, DOUT_GPIO_MODE_DOUT));   
}

/*****************************************************************************
 * Assign control of the digital output to counter timer
 * @param dev
 * @param dout
 */
static inline void _ct_out_assign(struct device *dev, uint8_t dout)
{
    dt78xx_fpga_rmw(dev, 
                    (dout < 4) ? FPGA_REG_DOUT_GPIO1 : FPGA_REG_DOUT_GPIO2, 
                    MASK_DOUT_GPIO_MODE(dout, ~DOUT_GPIO_MODE_DOUT), 
                    MASK_DOUT_GPIO_MODE(dout, DOUT_GPIO_MODE_CT));   
}
...
static int _fclose(struct inode *inode, struct file *file)
{
    ...
    _clear_ctr_intr(dev); 
    //disable digital output, if any, assigned to counter
    _ct_out_free(dev, ct->dout);
 ...
 */
static int _ct_cfg_get(struct dt7816_ct *ct, void __user *arg, size_t len)
{
    ...
    //get the ext clk and ext gate input pins
    regval = dt78xx_fpga_read(dev, FPGA_REG_DIN_GPIO3);
    cfg.ext_clk_din = (regval & MASK_DIN_GPIO3_CT_CLK) >> SHIFT_DIN_GPIO3_CT_CLK;
    cfg.ext_gate_din = (regval & MASK_DIN_GPIO3_CT_GATE) >> SHIFT_DIN_GPIO3_CT_GATE;
    
    if (mode == CT_CTRL_MODE_DIVIDER)
    {
        cfg.mode = ct_mode_divider;
        cfg.divider.out = ct->dout;
 ...
 * len : length of buffer pointed by arg
 */
static int _ct_cfg_set(struct dt7816_ct *ct, void __user *arg, size_t len)
{
    dt78xx_ct_config_t cfg;
    u16 regval;
...
#warning Measure && retriggerable 1shot modes NOT implemented yet   
    ...
    if (cfg.ext_clk)
    {
        regval |= (cfg.ext_clk_din << SHIFT_DIN_GPIO3_CT_CLK); 
        regval |= MASK_DIN_GPIO3_CT_CLK_EN;
    }
    //write the ext gate DIN configuration
    if (cfg.gate != ct_gate_none)
    {
        regval |= (cfg.ext_gate_din << SHIFT_DIN_GPIO3_CT_GATE);
        regval |= MASK_DIN_GPIO3_CT_GATE_EN;
    }
    dt78xx_fpga_rmw(dev, FPGA_REG_DIN_GPIO3, 
                    MASK_DIN_GPIO3_CT_GATE_EN | MASK_DIN_GPIO3_CT_GATE |
                    MASK_DIN_GPIO3_CT_CLK_EN | MASK_DIN_GPIO3_CT_CLK,
                    regval);
    ...
    if (cfg.mode == ct_mode_divider)
    {
        ...
        regval = CT_CTRL_MODE_DIVIDER; //control mode
        if (cfg.ext_clk)
            regval |= CT_CTRL_MODE_CLK_EXT;
        if (cfg.divider.out_hi)
            regval |= CT_CTRL_MODE_OUT_POL_HI;
        dt78xx_fpga_write(dev, FPGA_REG_CT_CTRL, regval);
        
        //write dout pin and enable output
        ct->dout = cfg.divider.out & (NUM_DOUT-1);
        _ct_out_assign(dev, ct->dout);
    }
    else if (cfg.mode == ct_mode_1shot)
    {
        ...
        //write dout pin and enable output
        ct->dout = cfg.one_shot.out & (NUM_DOUT-1);
        _ct_out_assign(dev, ct->dout);
    }
    else if (cfg.mode == ct_mode_counter)
    {
        ...
        regval = CT_CTRL_MODE_COUNTER; //control mode
        if (cfg.ext_clk)
            regval |= CT_CTRL_MODE_CLK_EXT;
        dt78xx_fpga_write(dev, FPGA_REG_CT_CTRL, regval);
 ...
 * len : unused
 */
static int _ct_start(struct dt7816_ct *ct, void __user *arg, int len)
{
    struct device *dev;
    u16 reg;
 ...
 * len : unused
 */
static int _ct_stop(struct dt7816_ct *ct, void __user *arg, int len)
{
    struct device *dev;
...
int dt7816_ct_subsystem_create(struct daq_subsystem *daq)
{
    ...
    subsys = devm_kzalloc(dev, sizeof(struct dt7816_ct), GFP_KERNEL);
    if (!subsys)
    {
        return -ENOMEM;
    }
                     ...
                     loff_t *offset)
{
    ...
    subsys = (struct dt7816_ct *)file_to_subsys(file);
    if (unlikely(!subsys))
        return -ENODEV;

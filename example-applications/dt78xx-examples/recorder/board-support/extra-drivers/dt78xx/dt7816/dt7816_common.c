 ...
 */
int dt7816_ioctl_led_get(struct device *dev, 
                         void __user *arg,
                         size_t len)
{
    uint16_t regval;
 ...
 * len : length of buffer pointed by arg
 */
int dt7816_ioctl_led_set(struct device *dev, 
                         void __user *arg, 
                         size_t len)
{
    dt78xx_led_t led_value;
    ...
    dt78xx_fpga_rmw(dev, FPGA_REG_LED, MASK_LED_ENABLE|led_value.mask, 
                    MASK_LED_ENABLE|led_value.state);

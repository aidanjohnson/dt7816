 ...
 * configures the board to sample analog channel 0. The sampled data is read
 * asynchronously from the input stream and written to a csv file.
 * 
...
#ifdef DT7837
"-g {1/10}  : AIN0 gain 1 or 10, default gain is 1 for +/-10V full scale\n"
#endif
};
...
int main (int argc, char** argv)
{
    ...
    for (i=0; i < numbuf; ++i)
    {
        ...
        {
#ifdef DT7837
            //AIN channels are 32-bits and always come first
            volt = raw2volts(*(int32_t *)raw, ain_cfg.gain); 
            fprintf(fd_data,"%6d, %.5f,%d", 
                    j+(i*samples_per_chan), volt, *(int32_t *)raw);
            raw += sizeof(uint32_t);
#elif DT7816

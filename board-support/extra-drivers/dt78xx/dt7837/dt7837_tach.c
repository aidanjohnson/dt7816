...
{
#if 0    
    struct dt7837_tach *subsys = file_to_subsys(file);
 ...
 */
static int _tach_cfg_get(struct dt7837_tach *tach, void __user *arg, size_t len)
{
    uint16_t regval;
 ...
 * len : length of buffer pointed by arg
 */
static int _tach_cfg_set(struct dt7837_tach *tach, void __user *arg, size_t len)
{
    uint16_t ctrl;
    dt78xx_tach_config_t cfg;
    struct device *dev;
...
static long _ioctl_cb(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    subsys = (struct dt7837_tach *)file_to_subsys(file);
    if (unlikely(!subsys))
        return -ENODEV;
...
int dt7837_tach_subsystem_create(struct daq_subsystem *daq)
{
    ...
    subsys = devm_kzalloc(dev, sizeof(struct dt7837_tach), GFP_KERNEL);
    if (!subsys)
    {
        return -ENOMEM;
    }

...
{
#if 0    
    struct dt7837_dout *subsys = file_to_subsys(file);
...
static long _ioctl_cb(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    subsys = (struct dt7837_dout *)file_to_subsys(file);
    if (unlikely(!subsys))
        return -ENODEV;
                       ...
                       loff_t *off)
{
    ...
    if ((ret=copy_from_user(&dout, buff, sizeof(dout))) != 0)
        return (ret);
...
/*****************************************************************************
 * Synchronous read from user application m
 */
...
int dt7837_dout_subsystem_create(struct daq_subsystem *daq)
{
    ...
    subsys = devm_kzalloc(dev, sizeof(struct dt7837_dout), GFP_KERNEL);
    if (!subsys)
    {
        return -ENOMEM;
    }

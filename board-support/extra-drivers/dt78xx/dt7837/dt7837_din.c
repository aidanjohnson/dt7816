...
//Data structure specific to the DT7837 digital input subsystem. 
struct dt7837_din
{
...
/**************************************************************************
 * File operations supported by DT7837 din subsystem
 */
...
static int _fopen(struct inode *inode, struct file *file)
{
    ...
    struct dt7837_din *subsys;
    int err;
    
    dt_dev = file_to_dt78xx(file);
    subsys = (struct dt7837_din *)file_to_subsys(file);
    if (!subsys)
    {
        return -ENODEV;
    }
...
#if 0    
    DEV_DBG(&dt_dev->pdev->dev, "[%s] %s %02x", __func__, subsys->filename, din);
#endif
...
int dt7837_din_subsystem_create(struct daq_subsystem *daq)
{
    ...
    subsys = devm_kzalloc(dev, sizeof(struct dt7837_din), GFP_KERNEL);
    if (!subsys)
    {
        return -ENOMEM;
    }

...
//File operations supported by DT7837 dout subsystem
...
{
#if 0    
    struct dt7837_mctr *subsys = file_to_subsys(file);
 ...
 */
static int _mctr_cfg_get(struct dt7837_mctr *mctr, void __user *arg, size_t len)
{
    uint16_t regval;
    ...
    cfg.stale_flag = (regval & MASK_MSR_CTR_CTRL_STALE) >> SHIFT_MSR_CTR_CTRL_STALE;
 ...
 * len : length of buffer pointed by arg
 */
static int _mctr_cfg_set(struct dt7837_mctr *mctr, void __user *arg, size_t len)
{
    uint16_t ctrl;
    dt78xx_mctr_config_t cfg;
    struct device *dev;
    ...
    // we'll be nice and filter out the values that are illegal in the config
    if ((cfg.stale_flag < 0) || (cfg.stale_flag > 1))
        return (-EINVAL);
    ...
    if ((cfg.stale_flag == 1) && (cfg.start_sel != cfg.stop_sel))
        return (-EINVAL);
    
    ctrl  = (cfg.stale_flag << SHIFT_MSR_CTR_CTRL_STALE) & MASK_MSR_CTR_CTRL_STALE;
    ctrl |= (cfg.start_sel << SHIFT_MSR_CTR_CTRL_START_SEL) & MASK_MSR_CTR_CTRL_START_SEL;
    ctrl |= (cfg.stop_sel << SHIFT_MSR_CTR_CTRL_STOP_SEL) & MASK_MSR_CTR_CTRL_STOP_SEL;
    dev = &mctr->dt_dev->pdev->dev;
...
static long _ioctl_cb(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    subsys = (struct dt7837_mctr *)file_to_subsys(file);
    if (unlikely(!subsys))
        return -ENODEV;
...
int dt7837_measure_subsystem_create(struct daq_subsystem *daq)
{
    ...
    subsys = devm_kzalloc(dev, sizeof(struct dt7837_mctr), GFP_KERNEL);
    if (!subsys)
    {
        return -ENOMEM;
    }
